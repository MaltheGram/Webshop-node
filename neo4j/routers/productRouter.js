...
router.post(`${produtcsgraph}`, async (req, res) => {
    ...
    try {
      const result = await createProductWithRelationToInventoryAndCategory(product, inventory, category);
...
router.get(`${produtcsgraph}/:id`, async (req, res) => {
    try {
        const productAndInventory = await getProductAndInventoryById(req.params.id);
...
router.put(`${produtcsgraph}/:id`, async (req, res) => {
    try {
        await updateProductAndInventory(req.params.id, req.body.product[0], req.body.inventory[0]);
...
router.get(`${produtcsgraph}/category/:categoryId`, async (req, res) => {
  ...
  router.put(`${produtcsgraph}/:productId/category/:newCategoryId`, async (req, res) => {
